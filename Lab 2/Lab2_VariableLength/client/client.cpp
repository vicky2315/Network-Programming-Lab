...
int main()
{
	...
	while (true)
	{
		printf("Type some text with the delimiter # (\"quit#\" to exit): ");
		fflush(stdout);
		...
		int servermessage = line.length();
		// Copy the line into the buffer, filling the rest with dashes.
		// (We must be careful not to write past the end of the array.)
		memset(buffer, '-', servermessage);
		memcpy(buffer, line.c_str(), servermessage);
		if (count(line.begin(), line.end(), '#') >= 1)
		{
			send(sock, buffer, servermessage - 1, 0);
		}
		else
		{
			printf("Please include the delimiter # in your message \n");
			continue;
		}
